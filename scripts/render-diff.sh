...
render_terragrunt_inputs() {
  ...
  # Check if file exists (handle new files)
  if [[ ! -f "$unit_file" ]]; then
    print_color "$YELLOW" "Note: Unit file $unit_file does not exist on branch $branch (likely new)"
    echo '{}' > "$output_file"
    return 0
  fi
...
compare_inputs() {
  ...
  pr_sorted="$TEMP_DIR/pr_$(basename "$pr_file" .json)_sorted.json"
  main_sorted="$TEMP_DIR/main_$(basename "$main_file" .json)_sorted.json"
...
generate_summary() {
  ...
  for unit in $(find_terragrunt_units "$PROJECT_ROOT"); do
    ...
    unit_name="$(basename "$(dirname "$unit")")/$(basename "$(dirname "$(dirname "$unit")")")"
    pr_file="$TEMP_DIR/pr_$(basename "$(dirname "$unit")")_$(basename "$(dirname "$(dirname "$unit")")").json"
